...
namespace Community.OData.Linq.Builder
{
    ...
    internal class EdmTypeBuilder
    {
        ...
        {
            this._configurations = configurations.ToList();
        }
        ...
        private Dictionary<Type, IEdmType> GetEdmTypes()
        {
            ...
            foreach (IEdmTypeConfiguration config in this._configurations)
            {
                this.CreateEdmTypeHeader(config);
            }

            foreach (IEdmTypeConfiguration config in this._configurations)
            {
                this.CreateEdmTypeBody(config);
            }

            foreach (StructuralTypeConfiguration structrual in this._configurations.OfType<StructuralTypeConfiguration>())
            {
                this.CreateNavigationProperty(structrual);
            }
        ...
        private void CreateEdmTypeHeader(IEdmTypeConfiguration config)
        {
            ...
            if (edmType == null)
            {
                if (config.Kind == EdmTypeKind.Complex)
                {
                    ...
                    {
                        this.CreateEdmTypeHeader(complex.BaseType);
                        baseType = this.GetEdmType(complex.BaseType.ClrType) as IEdmComplexType;
                    ...
                    this._types.Add(config.ClrType, complexType);

                    if (complex.IsOpen)
                    {
                        // add a mapping between the open complex type and its dynamic property dictionary.
                        this._openTypes.Add(complexType, complex.DynamicPropertyDictionary);
                    }
                    edmType = complexType;
                }
                else if (config.Kind == EdmTypeKind.Entity)
                {
                    ...
                    {
                        this.CreateEdmTypeHeader(entity.BaseType);
                        baseType = this.GetEdmType(entity.BaseType.ClrType) as IEdmEntityType;
                    ...
                    this._types.Add(config.ClrType, entityType);

                    if (entity.IsOpen)
                    {
                        // add a mapping between the open entity type and its dynamic property dictionary.
                        this._openTypes.Add(entityType, entity.DynamicPropertyDictionary);
                    }
                    edmType = entityType;
                }
                else
                {
                    ...
                    this._types.Add(enumTypeConfiguration.ClrType,
            ...
            if (structuredType != null && structuralTypeConfiguration != null &&
                !this._structuredTypeQuerySettings.ContainsKey(structuredType))
            {
                ModelBoundQuerySettings querySettings =
                    structuralTypeConfiguration.QueryConfiguration.ModelBoundQuerySettings;
                if (querySettings != null)
                {
                    this._structuredTypeQuerySettings.Add(structuredType,
                        structuralTypeConfiguration.QueryConfiguration.ModelBoundQuerySettings);
                }
            }
        }
            ...
            foreach (PropertyConfiguration property in config.Properties)
            {
                ...
                switch (property.Kind)
                {
                    ...
                    case PropertyKind.Complex:
                        ...
                        IEdmComplexType complexType = this.GetEdmType(complexProperty.RelatedClrType) as IEdmComplexType;
                    ...
                    case PropertyKind.Collection:
                        edmProperty = this.CreateStructuralTypeCollectionPropertyBody(type, (CollectionPropertyConfiguration)property);
                        break;

                    case PropertyKind.Enum:
                        edmProperty = this.CreateStructuralTypeEnumPropertyBody(type, (EnumPropertyConfiguration)property);
                        break;
                ...
                if (edmProperty != null)
                {
                    ...
                    {
                        this._properties[property.PropertyInfo] = edmProperty;
                    }

                    if (property.IsRestricted)
                    {
                        this._propertiesRestrictions[edmProperty] = new QueryableRestrictions(property);
                    }

                    if (property.QueryConfiguration.ModelBoundQuerySettings != null)
                    {
                        this._propertiesQuerySettings.Add(edmProperty, property.QueryConfiguration.ModelBoundQuerySettings);
                    }
                }
            }
        }
        ...
        private IEdmProperty CreateStructuralTypeCollectionPropertyBody(EdmStructuredType type, CollectionPropertyConfiguration collectionProperty)
        {
            ...
            if (clrType.IsEnum)
            {
                IEdmType edmType = this.GetEdmType(clrType);
        ...
        private IEdmProperty CreateStructuralTypeEnumPropertyBody(EdmStructuredType type, EnumPropertyConfiguration enumProperty)
        {
            ...
            IEdmType edmType = this.GetEdmType(enumPropertyType);
        ...
        private IList<IEdmStructuralProperty> GetDeclaringPropertyInfo(IEnumerable<PropertyInfo> propertyInfos)
        {
            ...
            foreach (PropertyInfo propInfo in propertyInfos)
            {
                ...
                if (this._properties.TryGetValue(propInfo, out edmProperty))
                {
                ...
                else
                {
                    ...
                    while (baseType != null)
                    {
                        ...
                        if (this._properties.TryGetValue(basePropInfo, out edmProperty))
                        {
                            edmProperties.Add(edmProperty);
                            break;
                        }
        ...
        private void CreateEnumTypeBody(EdmEnumType type, EnumTypeConfiguration config)
        {
            ...
            foreach (EnumMemberConfiguration member in config.Members)
            {
                ...
                this._members[member.MemberInfo] = edmMember;
            }
        }
