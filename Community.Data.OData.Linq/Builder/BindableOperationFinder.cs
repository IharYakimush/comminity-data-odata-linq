    ...
    internal class BindableOperationFinder
    {
        ...
        public BindableOperationFinder(IEdmModel model)
        {
            ...
            foreach (var operationGroup in operationGroups)
            {
                ...
                if (entityType != null)
                {
                    this._map[entityType] = operationGroup.ToList();
                }
                ...
                if (collectionType != null)
                {
                    ...
                    if (elementType != null)
                    {
                        ...
                        if (this._collectionMap.TryGetValue(elementType, out value))
                        {
                            value.AddRange(operationGroup);
                        }
                        else
                        {
                            this._collectionMap[elementType] = operationGroup.ToList();
                        }
                    }
                }
            }
        ...
        {
            return GetTypeHierarchy(entityType).SelectMany(this.FindDeclaredOperations);
        }
        ...
        {
            return GetTypeHierarchy(entityType).SelectMany(this.FindDeclaredOperationsBoundToCollection);
        }
        ...
        private IEnumerable<IEdmOperation> FindDeclaredOperations(IEdmEntityType entityType)
        {
            ...
            if (this._map.TryGetValue(entityType, out results))
            {
                return results;
            }
            else
        ...
        private IEnumerable<IEdmOperation> FindDeclaredOperationsBoundToCollection(IEdmEntityType entityType)
        {
            ...
            if (this._collectionMap.TryGetValue(entityType, out results))
            {
                return results;
            }
            else
