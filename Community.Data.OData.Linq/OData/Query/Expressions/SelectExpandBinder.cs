...
namespace Community.OData.Linq.OData.Query.Expressions
{
    ...
    /// <summary>
    /// Applies the given <see cref="IQueryable"/> to the given <see cref="SelectExpandQueryOption"/>.
    /// </summary>
    internal class SelectExpandBinder
    {
        ...
        public SelectExpandBinder(ODataQuerySettings settings, SelectExpandQueryOption selectExpandQuery)
        {
            ...
            this._selectExpandQuery = selectExpandQuery;
            this._context = selectExpandQuery.Context;
        ...
        private object Bind(object entity)
        {
            ...
            LambdaExpression projectionLambda = this.GetProjectionLambda();
        ...
        {
            Type elementType = this._selectExpandQuery.Context.ElementClrType;
        ...
        {
            Type elementType = this._selectExpandQuery.Context.ElementClrType;
            IEdmNavigationSource navigationSource = this._selectExpandQuery.Context.NavigationSource;
            ...
            Expression projectionExpression = this.ProjectElement(source, this._selectExpandQuery.SelectExpandClause, this._context.ElementType as IEdmEntityType, navigationSource);
            ...
            int? modelBoundPageSize = null)
        {
            ...
            if (source.Type.IsCollection(out elementType))
            {
                ...
                return this.ProjectCollection(source, elementType, selectExpandClause, entityType, navigationSource, expandedItem,
                    modelBoundPageSize);
            }
            else
            {
                // new Wrapper { v1 = source.property ... }
                return this.ProjectElement(source, selectExpandClause, entityType, navigationSource);
            }
        }
        ...
        internal Expression CreatePropertyNameExpression(IEdmEntityType elementType, IEdmProperty property, Expression source)
        {
            ...
            {
                Type originalType = EdmLibHelpers.GetClrType(elementType, this._model);
                Type castType = EdmLibHelpers.GetClrType(declaringType, this._model);
                if (castType == null)
                {
        ...
        internal Expression CreatePropertyValueExpression(IEdmEntityType elementType, IEdmProperty property, Expression source)
        {
            ...
            return this.CreatePropertyValueExpressionWithFilter(elementType, property, source, filterClause: null);
        }
            ...
            Expression source, FilterClause filterClause)
        {
            ...
            string propertyName = EdmLibHelpers.GetClrPropertyName(property, this._model);
            ...
            if (filterClause != null && property.Type.IsCollection())
            {
                ...
                Type clrElementType = EdmLibHelpers.GetClrType(edmElementType, this._model);
                ...
                Expression filterPredicate = FilterBinder.Bind(null, filterClause, clrElementType, this._context.RequestContainer);
        ...
        private Expression ProjectElement(Expression source, SelectExpandClause selectExpandClause, IEdmEntityType entityType, IEdmNavigationSource navigationSource)
        {
            ...
            wrapperTypeMemberAssignments.Add(Expression.Bind(wrapperProperty, Expression.Constant(this._modelID)));
            ...
            else
            {
                ...
                Expression typeName = CreateTypeNameExpression(source, entityType, this._model);
                if (typeName != null)
                {
            ...
            if (selectExpandClause != null)
            {
                ...
                ISet<IEdmStructuralProperty> propertiesToInclude = GetPropertiesToIncludeInQuery(selectExpandClause, entityType, navigationSource, this._model, out autoSelectedProperties);
                ...
                if (propertiesToExpand.Count > 0 || propertiesToInclude.Count > 0 || autoSelectedProperties.Count > 0)
                {
                    ...
                    Expression propertyContainerCreation =
                        this.BuildPropertyContainer(entityType, source, propertiesToExpand, propertiesToInclude, autoSelectedProperties, isSelectingOpenTypeSegments);
            ...
            ISet<IEdmStructuralProperty> propertiesToInclude, ISet<IEdmStructuralProperty> autoSelectedProperties, bool isSelectingOpenTypeSegments)
        {
            ...
            foreach (KeyValuePair<IEdmNavigationProperty, ExpandedNavigationSelectItem> kvp in propertiesToExpand)
            {
                ...
                Expression propertyName = this.CreatePropertyNameExpression(elementType, propertyToExpand, source);
                Expression propertyValue = this.CreatePropertyValueExpressionWithFilter(elementType, propertyToExpand, source,
                    expandItem.FilterOption);
                Expression nullCheck = this.GetNullCheckExpression(propertyToExpand, propertyValue, projection);

                Expression countExpression = this.CreateTotalCountExpression(propertyValue, expandItem);
                ...
                if (projection != null)
                {
                    ...
                    propertyValue = this.ProjectAsWrapper(propertyValue, projection, propertyToExpand.ToEntityType(), expandItem.NavigationSource, expandItem, modelBoundPageSize);
                }
                ...
                if (projection != null)
                {
                    ...
                    {
                        propertyExpression.PageSize = this._settings.PageSize.Value;
                    }
                    else
                    {
            ...
            {
                Expression propertyName = this.CreatePropertyNameExpression(elementType, propertyToInclude, source);
                Expression propertyValue = this.CreatePropertyValueExpression(elementType, propertyToInclude, source);
            ...
            {
                var dynamicPropertyDictionary = EdmLibHelpers.GetDynamicPropertyDictionary(elementType, this._model);
        ...
        private Expression AddOrderByQueryForSource(Expression source, OrderByClause orderbyClause, Type elementType)
        {
            if (orderbyClause != null)
            {
                ...
                LambdaExpression orderByExpression =
                    FilterBinder.Bind(null, orderbyClause, elementType, this._context.RequestContainer);
            ...
            SelectExpandClause projection)
        {
            ...
            foreach (var key in propertyToExpand.ToEntityType().Key())
            {
                var keyExpression = Expression.Equal(
                    this.CreatePropertyValueExpressionWithFilter(propertyToExpand.ToEntityType(), key, propertyValue, null),
        ...
        private Expression ProjectCollection(Expression source, Type elementType, SelectExpandClause selectExpandClause, IEdmEntityType entityType, IEdmNavigationSource navigationSource, ExpandedNavigationSelectItem expandedItem, int? modelBoundPageSize)
        {
            ...
            // expression
            //      new Wrapper { }
            Expression projection = this.ProjectElement(element, selectExpandClause, entityType, navigationSource);
            ...
            {
                source = this.AddOrderByQueryForSource(source, expandedItem.OrderByOption, elementType);
            }

            if (this._settings.PageSize.HasValue || modelBoundPageSize.HasValue ||
                (expandedItem != null && (expandedItem.TopOption.HasValue || expandedItem.SkipOption.HasValue)))
            {
                ...
                if (expandedItem != null && expandedItem.SkipOption.HasValue)
                {
                    ...
                    source = ExpressionHelpers.Skip(source, (int)expandedItem.SkipOption.Value, elementType,
                        this._settings.EnableConstantParameterization);
                }
                ...
                if (this._settings.PageSize.HasValue)
                {
                    source = ExpressionHelpers.Take(source, this._settings.PageSize.Value + 1, elementType,
                        this._settings.EnableConstantParameterization);
                }
                else if (this._settings.ModelBoundPageSize.HasValue)
                {
